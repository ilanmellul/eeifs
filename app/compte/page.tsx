...
const typeConfig: Record<PostType, { icon: typeof Camera; label: string; bg: string; text: string }> = {
  photo:      { icon: Camera,      label: 'Photo',      bg: 'bg-violet-100', text: 'text-violet-600' },
  info:       { icon: Info,        label: 'Info',        bg: 'bg-sky-100',    text: 'text-sky-600'    },
  programme:  { icon: CalendarDays,label: 'Programme',  bg: 'bg-amber-100',  text: 'text-amber-600'  },
}
...
export default async function ComptePage() {
  ...
  const gradient = roleBg[profile.role] ?? 'from-orange-400 to-rose-500'
  ...
  return (
    <main className="max-w-2xl mx-auto px-4 py-8 space-y-5">
      ...
      <div>
        ...
        {posts.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-3xl border border-orange-50 shadow-sm">
          ...
          <div className="space-y-3">
            {posts.map((post) => {
              const { icon: Icon, label, bg, text } = typeConfig[post.type as PostType]
              ...
              return (
                <div key={post.id} className="bg-white rounded-2xl border border-orange-50 shadow-sm overflow-hidden">
                  <div className="flex items-stretch">
                    {/* Left accent bar */}
                    <div className={`w-1.5 ${bg} shrink-0`} />
